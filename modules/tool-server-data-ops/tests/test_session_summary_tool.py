"""Tests for SessionSummaryTool.

Constructor: SessionSummaryTool(coordinator, resolver=None). Patch path is
amplifier_module_tool_server_data_ops.session_summary_tool.
"""
...
def _make_mock_async_ci_client(return_value: Any = None):
    ...
    mock_instance.session_summary = AsyncMock(
        return_value=return_value if return_value is not None else {"deletable": True}
    )
...
# ---------------------------------------------------------------------------
# TestSessionSummaryToolProtocol
# ---------------------------------------------------------------------------


class TestSessionSummaryToolProtocol:
    """Tool protocol surface tests."""

    def test_name_is_session_summary(self) -> None:
        from amplifier_module_tool_server_data_ops.session_summary_tool import SessionSummaryTool

        tool = SessionSummaryTool(_make_coordinator())
        assert tool.name == "session_summary"

    def test_description_mentions_preview(self) -> None:
        from amplifier_module_tool_server_data_ops.session_summary_tool import SessionSummaryTool

        tool = SessionSummaryTool(_make_coordinator())
        assert "preview" in tool.description.lower()
    ...
    async def test_execute_returns_tool_result(self) -> None:
        ...
        tool = SessionSummaryTool(coordinator)
...
class TestListSources:
    async def test_list_sources_does_not_call_client(self) -> None:
        ...
        tool = SessionSummaryTool(coordinator, resolver)
...
# ---------------------------------------------------------------------------
# TestSessionSummaryConstruction -- AsyncCIClient construction and delegation
# ---------------------------------------------------------------------------
...
class TestSessionSummaryConstruction:
    ...
    async def test_session_id_forwarded_to_client_session_summary(self) -> None:
        from amplifier_module_tool_server_data_ops.session_summary_tool import SessionSummaryTool
        ...
        mock_instance.session_summary.assert_called_once_with("the-session-id")
    ...
    async def test_result_forwarded_and_source_stamped(self) -> None:
        ...
        expected = {"created_by": "alice", "node_count": 10, "deletable": True}
...
# ---------------------------------------------------------------------------
# TestSessionSummaryConfigFallback
# ---------------------------------------------------------------------------


class TestSessionSummaryConfigFallback:
...
# ---------------------------------------------------------------------------
# TestSessionSummarySourceSelection -- pool/selection + fail-loud ambiguity
# ---------------------------------------------------------------------------


class TestSessionSummarySourceSelection:
...
# ---------------------------------------------------------------------------
# TestSessionSummaryServerErrors -- 404/409 surfaced as clear tool errors
# ---------------------------------------------------------------------------


class TestSessionSummaryServerErrors:
    async def test_404_surfaces_as_clear_tool_error(self) -> None:
        ...
        mock_instance.session_summary = AsyncMock(
            side_effect=CIClientError(
                "HTTP 404 from http://ci-server:9000/sessions/missing/summary",
                error_type="http_status",
                url="http://ci-server:9000/sessions/missing/summary",
                status_code=404,
            )
        )
        mock_cls = MagicMock(return_value=mock_instance)
    ...
    async def test_409_surfaces_as_clear_tool_error(self) -> None:
        ...
        mock_instance.session_summary = AsyncMock(
            side_effect=CIClientError(
                "HTTP 409 from http://ci-server:9000/sessions/live/summary",
                error_type="http_status",
                url="http://ci-server:9000/sessions/live/summary",
                status_code=409,
            )
        )
        mock_cls = MagicMock(return_value=mock_instance)
